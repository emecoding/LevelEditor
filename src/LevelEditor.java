...
public class LevelEditor
{
    ...
    public LevelEditor(String textures_dir, String final_dir, String final_file_name) throws IOException
    {
        ...
        this.m_camera = new Camera(new Vector2f(0.0f, 0.0f), this.m_renderer);
    ...
    private void use_sprite_buttons()
    {
        ...
        for(int i = 0; i < this.m_textureManager.TEXTURES.size(); i++)
        {
            ...
            Integer[] tex = this.m_textureManager.TEXTURES.get(i);
            ...
            if(Window.collides(rect1, rect2))
            {
                ...
                {
                    this.m_current_item = new float[]{(float)i, (float)tex[1], (float)tex[2], 0.0f};
                }
    ...
    private void use_current_item()
    {
        ...
        Integer[] tex = this.m_textureManager.TEXTURES.get((int)this.m_current_item[0]);
        ...
        if(Window.mouse_button_is_pressed(GLFW_MOUSE_BUTTON_RIGHT) && !just_pressed_right_mouse_button)
        {
            ...
            float[] i = new float[]{tex[0]-2, (int)x - Camera.CAMERA_OFF_SET_X, (int)y-Camera.CAMERA_OFF_SET_Y, this.m_current_item[1], this.m_current_item[2], this.m_current_item[3]};
            this.MAP.add(i);
        }
    ...
    private void render_map()
    {
        for(int i = 0; i < this.MAP.size(); i++)
        {
            ...
            Integer[] tex = this.m_textureManager.TEXTURES.get((int)item[0]);
            Vector2f mouse_pos = Window.get_mouse_position();

            float[] rect1 = new float[]
                    {
                            item[1] + Camera.CAMERA_OFF_SET_X, item[2] + Camera.CAMERA_OFF_SET_Y, item[3], item[4]
                    };
    ...
    private void save() throws IOException
    {
        ...
        for(int i = 0; i < this.MAP.size(); i++)
        {
            float[] item = this.MAP.get(i);
            Integer[] tex = this.m_textureManager.TEXTURES.get((int)item[0]);
            String data = "[" + item[1] + "," + item[2] + "," + item[3] + "," + item[4] + "," + item[5] + "," + (int)item[0] + "]\n";
    ...
    private void load_old_map() throws FileNotFoundException
    {
        ...
        for(int i = 0; i < lines.size(); i++)
        {
            ...
            float x = Float.parseFloat(splitted_line[0]);
            float y = Float.parseFloat(splitted_line[1]);
            float width = Float.parseFloat(splitted_line[2]);
            float height = Float.parseFloat(splitted_line[3]);
            float rotation = Float.parseFloat(splitted_line[4]);
            int tex_id = Integer.parseInt(splitted_line[5]);
